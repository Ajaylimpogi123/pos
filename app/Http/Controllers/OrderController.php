...
class OrderController extends Controller
{
    public function store(Request $request): RedirectResponse
    {
        try {
            ...
            // Regenerate on collision — mt_rand alone doesn't guarantee
            // uniqueness. queue_no is already guaranteed unique per day (via
            // the lock above), so it's used as a deterministic fallback if
            // random generation keeps colliding.
            $invoiceNo = 'INV-'.date('Ymd').'-'.str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
            $attempts = 0;
            while (Order::where('invoice_no', $invoiceNo)->exists() && $attempts < 5) {
                $invoiceNo = 'INV-'.date('Ymd').'-'.str_pad(mt_rand(1, 99999), 5, '0', STR_PAD_LEFT);
                $attempts++;
            }
            if (Order::where('invoice_no', $invoiceNo)->exists()) {
                $invoiceNo = 'INV-'.date('Ymd').'-'.str_pad($queueNo, 5, '0', STR_PAD_LEFT);
            }
            ...
            // Deliberately its own try/catch: this runs after the order has
            // already committed successfully, so a print/enqueue failure
    ...
    {
        if (config('printer.mode') === 'direct') {
            return $this->printKitchenDirect();
        }

        return $this->printKitchenQueued();
    }

    /**
     * Original synchronous ESC/POS-over-LAN kitchen print path — unchanged
     * behavior, used whenever config('printer.mode') === 'direct' (the
     * default, for on-prem/LAN deployments that can actually reach the
     * printers from the web process).
     */
    private function printKitchenDirect()
    {
    ...
    /**
     * Queued/bridged kitchen print path — used when config('printer.mode')
     * === 'queue' (VPS deployment with no route to the LAN printers). Same
     * unprinted-qty diff logic as printKitchenDirect(), but instead of
     * printing synchronously it creates PrintJob rows (one per target
     * printer) for the LAN print agent to pick up. ct_printed_qty is
     * deliberately NOT touched here — that only happens later, via
     * PrintJobService::markCartRowsPrinted(), once the agent reports a
     * successful print.
     */
    private function printKitchenQueued()
    {
        try {
            $cartRows = Cart::with('product:pd_id,pd_name,cat_id')->get();
        } catch (\Exception $e) {
        ...
        try {
            $jobs = app(\App\Services\PrintJobService::class)->enqueueKitchen($newItems, $ticketTag);
        } catch (\Exception $e) {
            Log::error('Failed to enqueue kitchen print job(s): '.$e->getMessage(), [
                'items' => $newItems,
            ]);

            return redirect()->back()->with('error', 'Failed to queue kitchen ticket: '.$e->getMessage());
        }

        return redirect()->back()
            ->with('success', 'Kitchen ticket queued for printing.')
            ->with('print_jobs', $jobs->pluck('pj_id'));
    }
}
