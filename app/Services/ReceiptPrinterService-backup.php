...
class ReceiptPrinterService
{
    ...
    /**
     * Print a pre-order kitchen ticket straight from cart items, before
     * the sale is confirmed. No Order/OrderItems rows exist yet (checkout
     * hasn't happened), so this builds its own minimal ticket instead of
     * reusing buildReceipt(), which requires a persisted Order. No pricing
     * shown — kitchen doesn't need it, and totals aren't final until
     * checkout is confirmed.
     *
     * @param array  $items       [['pd_id' => int, 'pd_name' => string, 'ct_qty' => int, 'cat_id' => int|null], ...]
     * @param string $tableNumber
     * @param bool   $routeByCategory  If true, split items across printers using
     *                                 config('printer.category_routing') (requires
     *                                 'cat_id' present in each item array). If false,
     *                                 everything goes to a single printer.
     * @param string|null $printerName Printer profile to use when $routeByCategory is false.
     *                                  Defaults to 'kitchen'.
     * @return array<string,bool> Map of printer name => success, for inspection by the caller.
     */
   ...
   /**
 * Print a kitchen ticket straight from cart items, before the sale is
 * confirmed. No Order/OrderItems rows exist yet, so this builds its own
 * minimal ticket instead of reusing buildReceipt(), which requires a
 * persisted Order. No pricing shown — kitchen doesn't need it.
 */
public function printKitchenTicket(array $items, string $tableNumber, ?string $printerName = null): bool
{
    $printerName ??= 'kitchen';
    $profile = config("printer.printers.{$printerName}");

    if (!$profile || !($profile['enabled'] ?? false)) {
        Log::warning("Printer '{$printerName}' is disabled in config, skipping kitchen print.", [
    ...
    if (!$connector) {
        Log::warning("Printer '{$printerName}' not reachable, skipping kitchen print.", [
            'table_number' => $tableNumber,
        ]);
        return false;
    }
    ...
    try {
        ...
        Log::error("Kitchen print to '{$printerName}' failed: " . $e->getMessage(), [
            'table_number' => $tableNumber,
        ]);
        return false;
    }
